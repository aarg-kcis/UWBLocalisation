...
int pollCount = 0;
int anchorId = 0;
const int noOfAnchors = 4; // To change
int tagId = 21;
bool start = true;
...
int runCommand() {
  ...
  case 'd':
    Serial.println( String(distances_arr[0], 4) + ' ' + String(distances_arr[1], 4) + ' ' + String(distances_arr[2], 4) + ' ' + String(distances_arr[3], 4) );
...
void loop()
{
  ...
  while (true)
  {
    if (millis() - lastActivity > 250) //Subhasis: new change here
    {
      start = true;
      break;
    }
    ...
    {
      receivedAck = false;
      noteActivity(); //Subhasis: New Change here
      DW1000.getData(data, LEN_DATA);
      if (data[16] == 4 && data[17] == 23 && data[0] == RANGE_REPORT)
      {
        start = true;
      }
    }
